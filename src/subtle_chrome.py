...
# We'll replace the .chrome-text block completely again.
chrome_css = r'''
        .chrome-text, h1, h2, p, .subtitle, .disclaimer, .phone, .quote {
            background: linear-gradient(to bottom right, #ffffff 0%, #f1f5f9 20%, #e2e8f0 45%, #ffffff 50%, #f8fafc 75%, #ffffff 100%);
        ...
        .menu-item, .btn-enter, .btn-back {
            background: linear-gradient(to bottom right, #ffffff 0%, #e2e8f0 45%, #ffffff 50%, #f1f5f9 75%, #ffffff 100%);
            -webkit-background-clip: text;
            background-clip: text;
            color: transparent !important;
            text-shadow: 0px 1px 2px rgba(0,0,0,0.5);
...
for f_name in html_files:
    ...
    if f_name == 'index.html':
        ...
        content = re.sub(r'gradient\.addColorStop\(0\.25, "[^"]+"\);', r'gradient.addColorStop(0.25, "#f1f5f9");', content)
